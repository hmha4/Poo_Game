...
BOOL CGraphics::Initialize(HWND hWnd, BOOL windowed)
{
	...
	HRESULT hresult = 0;
	//Create the device
	hresult = m_pD3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd,
		vertexProcessing, &m_pp, &m_pDevice);
	if (FAILED(hresult))
	{
		SAFE_RELEASE(m_pD3D9);
...
//-----------------Resets the device------------------
//Purpose: whenever the window is resized, the device is lost, 
//         or we toggle between window and fullscreen mode
//Returns: TRUE on success, FALSE on failure
//-----------------------------------------------------
